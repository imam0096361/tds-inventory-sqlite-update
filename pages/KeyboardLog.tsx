...
export const KeyboardLog: React.FC = () => {
    ...
    useEffect(() => {
        fetch('http://localhost:3001/api/keyboardlogs')
            .then(res => res.json())
    ...
    const handleConfirmDelete = async () => {
        ...
        await fetch(`http://localhost:3001/api/keyboardlogs/${logToDelete.id}`, { method: 'DELETE' });
    ...
    const handleSave = async () => {
        if (editingLog) {
            await fetch(`http://localhost:3001/api/keyboardlogs/${editingLog.id}`, {
                method: 'PUT',
            ...
            await fetch('http://localhost:3001/api/keyboardlogs', {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
