...
export const PortableHDDLog: React.FC = () => {
    ...
    useEffect(() => {
        apiFetch<PeripheralLogEntry[]>('/api/portablehddlogs')
    ...
    const handleConfirmDelete = async () => {
        ...
        await apiFetch(`/api/portablehddlogs/${logToDelete.id}`, { method: 'DELETE' });
    ...
    const handleSave = async () => {
        if (editingLog) {
            await apiFetch(`/api/portablehddlogs/${editingLog.id}`, {
                method: 'PUT',
            ...
            await apiFetch('/api/portablehddlogs', {
                method: 'POST',
    ...
    const handleExport = () => {
        exportToCSV(sortedLogs, 'portable-hdd-service-logs');
    };
    ...
    const handleImportLogs = async (newLogs: Partial<PeripheralLogEntry>[]) => {
        ...
        const addPromises = logsToAdd.map(log => apiFetch('/api/portablehddlogs', {
            method: 'POST',
        ...
        <>
            <div className="bg-white p-6 rounded-xl shadow-lg">
                <div className="flex flex-col md:flex-row justify-between md:items-center mb-4 gap-4">
                    <h1 className="text-2xl font-bold text-gray-800">Portable HDD Service Log</h1>
            ...
            <ImportModal<Partial<PeripheralLogEntry>>
                ...
                assetName="Portable HDD Logs"
                templateHeaders={['productName', 'serialNumber', 'pcName', 'pcUsername', 'department', 'date', 'time', 'servicedBy', 'comment']}
                exampleRow={['Seagate Backup Plus 2TB', 'SN987654', 'IT-PC-04', 'sarah.wilson', 'IT', '2024-10-24', '14:00', 'Tech Support', 'HDD data recovery']}
            />
        </>
    );
};
