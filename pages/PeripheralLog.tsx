...
export const PeripheralLog: React.FC = () => {
    ...
    useEffect(() => {
        apiFetch<PeripheralLogEntry[]>('/api/mouselogs')
    ...
    const handleConfirmDelete = async () => {
        ...
        await apiFetch(`/api/mouselogs/${logToDelete.id}`, { method: 'DELETE' });
    ...
    const handleSave = async () => {
        if (editingLog) {
            await apiFetch(`/api/mouselogs/${editingLog.id}`, {
                method: 'PUT',
    ...
    const handleImportLogs = async (newLogs: Partial<PeripheralLogEntry>[]) => {
        ...
        const addPromises = logsToAdd.map(log => apiFetch('/api/mouselogs', {
            method: 'POST',
