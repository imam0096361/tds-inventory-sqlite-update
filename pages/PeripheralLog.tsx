...
export const PeripheralLog: React.FC = () => {
    ...
    const handleConfirmDelete = async () => {
        ...
        await fetch(buildApiUrl(`/api/mouselogs/${logToDelete.id}`), { method: 'DELETE' });
    ...
    const handleSave = async () => {
        if (editingLog) {
            await fetch(buildApiUrl(`/api/mouselogs/${editingLog.id}`), {
                method: 'PUT',
            ...
            await fetch(buildApiUrl('/api/mouselogs'), {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
    ...
    const handleImportLogs = async (newLogs: Partial<PeripheralLogEntry>[]) => {
        ...
        const addPromises = logsToAdd.map(log => fetch(buildApiUrl('/api/mouselogs'), {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
